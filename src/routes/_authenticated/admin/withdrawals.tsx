...
const fmtKes = (cents: number, cur = "KES") =>
  `${cur} ${(cents / 100).toLocaleString("en-KE", { minimumFractionDigits: 2 })}`;
...
function AdminWithdrawals() {
  ...
  // ── Approve & dispatch B2C via SasaPay ──────────────────────────────────
  const approve = async (w: WithdrawalRow) => {
    ...
    try {
      // Atomically claim the row → processing (prevents double-approvals)
      ...
      if (claimRes.error || !claimRes.data) {
        const { data: current } = await (supabase.from("withdrawals") as any)
          .select("status").eq("id", w.id).single();
        toast.error(`Cannot approve — already ${(current as any)?.status ?? "processing"}`);
        return;
      }

      if (!w.phone) {
        // No phone on file — mark completed manually
        await Promise.all([
      ...
      // Build a unique merchant transaction reference and store it so the callback can match
      const merchantTransRef = `SD-WD-${w.id.slice(0, 8)}-${Date.now()}`;
      await (supabase.from("withdrawals") as any)
        .update({ provider_ref: merchantTransRef })
        .eq("id", w.id);

      // Dispatch SasaPay B2C
      try {
  ...
  const reject = async (w: WithdrawalRow) => {
    ...
    try {
      const { data: fresh } = await (supabase.from("withdrawals") as any)
        .select("status").eq("id", w.id).single();
      if ((fresh as any)?.status !== "pending") {
        toast.error(`Cannot reject — already ${(fresh as any)?.status}`);
        return;
      }
      const { data: walletData } = await supabase.from("wallets")
        .select("balance_cents").eq("id", w.wallet_id).single();
      const wallet = walletData as { balance_cents: number } | null;
      if (!wallet) throw new Error("Wallet not found");

      await Promise.all([
        (supabase.from("wallets") as any).update({
        ...
        (supabase.from("withdrawals") as any).update({
          status:     "cancelled",
          updated_at: new Date().toISOString(),
  ...
  const refundWithdrawal = async (w: WithdrawalRow, reason: string) => {
    const { data: walletData } = await supabase.from("wallets")
      .select("balance_cents").eq("id", w.wallet_id).single();
    const wBal = (walletData as any)?.balance_cents ?? 0;
    await Promise.all([
      (supabase.from("wallets") as any).update({
        ...
        {FILTERS.map(s => (
          ...
          >
            ...
            {s === "pending" && pendingCount > 0 && (
              <span className="ml-1.5 inline-flex size-4 items-center justify-center rounded-full bg-warning text-black text-[9px] font-bold">
                {pendingCount}
              </span>
            )}
          </button>
        ...
        <>
          ...
          <div className="hidden md:block rounded-2xl border border-border/60 bg-gradient-surface shadow-card overflow-hidden">
            <table className="w-full text-sm">
              ...
              <tbody>
                {rows.map(w => (
                  <tr key={w.id} className="border-b border-border/25 hover:bg-surface/30 transition-colors last:border-0">
                    <td className="px-5 py-3 font-mono text-xs text-muted-foreground">
                      {w.user_id.slice(0, 12)}…
                    </td>
                    <td className="px-4 py-3 font-mono font-semibold text-loss">
                      −{fmtKes(w.amount_cents, w.currency)}
                    </td>
                    <td className="px-4 py-3 text-xs">{w.phone ?? "—"}</td>
                    <td className="px-4 py-3">
                      <Badge variant="outline" className={`text-[10px] capitalize ${STATUS_COLOR[w.status] ?? ""}`}>
                        {w.status}
                      </Badge>
                    </td>
                    ...
                    <td className="px-5 py-3 text-right">
                      ...
                      {w.status === "completed" && w.provider_ref && (
                        <span className="text-xs text-muted-foreground font-mono">
                          {w.provider_ref.slice(0, 14)}…
                        </span>
                      )}
                    </td>
                  </tr>
          ...
          <div className="md:hidden space-y-2">
            {rows.map(w => (
              <div key={w.id} className="rounded-xl border border-border/50 bg-gradient-surface p-4 space-y-3 shadow-card">
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <p className="font-mono font-bold text-base text-loss">
                      −{fmtKes(w.amount_cents, w.currency)}
                    </p>
                    <p className="text-xs text-muted-foreground mt-0.5">{w.phone ?? "—"}</p>
                  </div>
                  <Badge variant="outline" className={`text-[10px] capitalize shrink-0 ${STATUS_COLOR[w.status] ?? ""}`}>
                    {w.status}
                  </Badge>
                </div>
                ...
                <div className="flex items-center justify-between text-[10px] text-muted-foreground">
                  ...
                  <span className="shrink-0">{fmtDate(w.created_at)}</span>
                </div>
                ...
                {w.status === "processing" && (
                  <div className="pt-1 border-t border-border/30">
                    <p className="text-xs text-primary flex items-center gap-1.5">
                      <Loader2 className="size-3 animate-spin" />
                      B2C dispatched — awaiting SasaPay confirmation
                    </p>
                  </div>
                )}
              </div>
